...
function removeLastOccurrences(str, value) {
  ...
  return i < 0 ? str : str.substring(0, i) + str.substring(i + value.length);
}
...
function sumOfCodes(str) {
  return !str
    ? 0
    : str.split('').reduce((sum, _x, i) => sum + str.charCodeAt(i), 0);
}
...
function isPalindrome(str) {
  return str
    ...
    .split('')
    .every((c, i, arr) =>
      i === Math.floor(str.length / 2) ? true : c === arr[arr.length - (i + 1)]
    );
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .find(
      (l) => l.length === Math.max(...sentence.split(' ').map((w) => w.length))
    );
}
